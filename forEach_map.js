...
showFirstAndLast(["amy", "bruce", "dana", "frank"]);
...
addKeyAndValue(
  [{ name: "Jack" }, { name: "Dawson" }, { name: "Nico" }, { name: "Timo" }],
  "title",
  "player"
);
...
function doubleValuesWithMap(arr) {
  return arr.map(function (val) {
    let iO = arr.indexOf(val);
    return val * iO;
  });
}
valTimesIndex([9, 2, 5, 1]);
...
extractFullName([
  { first: "Jack", last: "Hughes" },
  { first: "Nico", last: "Hischier" },
  { first: "Miles", last: "Wood" },
  { first: "Dawson", last: "Mercer" },
]);
